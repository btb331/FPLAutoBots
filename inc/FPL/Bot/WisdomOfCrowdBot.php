...
class WisdomOfCrowdBot extends Bot
{
	...
	private function get_squad_with_net_transfers(){
		...
		foreach ($squad as $player) {
			...
			$squadTransfers[$type][$player] = $net;
		}
	...
	protected function make_transfers()
	{
		...
		foreach ($data as $player => $pData) {
			...
			$transfersOut = $pData['transfers_out_event'];
			$net = $transfersIn - $transfersOut;
			$playerTransfers[$player] = $net;
		}

		arsort($playerTransfers);
		...
		for($i=0; $i<count($squadTransfers); $i++){
			asort($squadTransfers[$i]);
		}
		...
		while(!$validTransfer){
			...
			$playersOrdered = array_keys($playerTransfers);
			$playerTransferIn = $playersOrdered[$playerTry];
			$playerInType = $data[$playerTransferIn]['element_type'];
			$playerOut = array_keys($squadTransfers[$playerInType])[$count];

			echo "Trying in transfer in " . $data[$playerTransferIn]['web_name'] . "...";
			if(in_array($playerTransferIn, $squad)){
				echo "already have him! \n";
				$playerTry++;
				continue;
			}

			$isValidTransfer = $this->_FPL->is_transfer_valid($playerOut, $playerTransferIn);
			...
			if($isValidTransfer){
				...
				echo $data[$playerTransferIn]['web_name'] . " choosen to add" . "\n";
				$this->_FPL->transfer([$playerOut], [$playerTransferIn]);
				echo "Transfer done \n\n";
			}else{
				echo "...falied \n";
				if($count>=count($squadTransfers[$playerInType])-1){
					$playerTry++;
					$count=0;
				}else{
						$count++;
			...
			if($playerTry>11){
				...
				$this->generateMessage($message, "Wisdom Of Crowd Bot 1");
				return;

			}

		}
		$message = "Your fellow humans mostly bought " . $data[$playerTransferIn]['web_name'] . " so I did too and decided to sell " . $data[$playerOut]['web_name'] . ".";
